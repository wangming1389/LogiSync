...
export class AccountCreatedConsumer implements OnModuleInit {
	...
	private async handleMessage(raw: unknown): Promise<void> {
		...
		await this.emailService.sendMail({
			to: raw.email,
			subject: `[LogiSync] Tài khoản nhân viên ${raw.workspaceName} đã được tạo`,
			text: [
				`Xin chào ${greetingName},`,
				'',
				`Tài khoản LogiSync của bạn cho workspace "${raw.workspaceName}" đã được tạo.`,
				`Email đăng nhập: ${raw.email}`,
				`Mật khẩu tạm thời: ${raw.tempPassword}`,
				'',
				'Vui lòng đăng nhập và đổi mật khẩu ngay trong lần đầu tiên truy cập.',
				'',
				'-- LogiSync',
			].join('\n'),
		});
