...
export default function SupplierCatalogClient() {
	...
	const categoryProductCounts = useMemo(() => {
		...
		for (const product of products) {
			counts.set(
				...
				(counts.get(product.supplierCategoryId) ?? 0) + 1,
			);
		}
		return counts;
	}, [products]);
	...
	const filteredProducts = products.filter(
		(product) =>
			(product.name.toLowerCase().includes(search.toLowerCase()) ||
				...
				product.uomLabel.toLowerCase().includes(search.toLowerCase())) &&
	...
	async function loadMasterData(): Promise<MasterData> {
		setLoadError('');
		const [categoryResult, catalogCategoryResult, uomResult] =
	...
	async function loadProducts(masterData?: MasterData) {
		...
		try {
			...
			const categoryMap = new Map(
				(masterData?.categoryItems ?? supplierCategories).map((item) => [
					item.id,
					item.name,
				]),
			);
			const uomMap = new Map(
				(masterData?.uomItems ?? uoms).map((item) => [item.id, item]),
			);
	...
	async function uploadProductImage(productId: string) {
		...
		await uploadProductImageFile(productId, selectedImageFile, true);
	}
	...
	async function saveProduct() {
		...
		if (supplierCategories.length === 0) {
			...
			if (existingSupplierCategory) {
				...
				const createdCategory: any = await createSupplierCategory({
					...
					<div className="flex gap-3 mb-4">
						<div className="relative flex-1">
							...
							<input
								...
								placeholder="Search product name, SKU or UoM..."
								...
								{isLoading ? (
									...
									filteredProducts.map((product, index) => (
										...
										>
											...
											>
												{product.stock.toLocaleString()} {product.uomLabel}
											</td>
											<td className="px-4 py-3">
							...
							{supplierCategories.length === 0 ? (
								...
								supplierCategories.map((category, index) => (
									...
									>
										...
										>
											{catalogCategoryById.get(category.catalogCategoryId)
												?.name ?? '—'}
										</td>
										<td
											className="px-5 py-3"
					...
					>
						...
						<div className="space-y-4">
							<div>
								...
								<select
									value={categoryForm.catalogCategoryId}
									onChange={(event) =>
										setCategoryForm({
											...categoryForm,
								...
								>
									{catalogCategories.length === 0 ? (
										<option value="">No master categories</option>
									) : null}
							...
							<div>
								...
								<textarea
									...
									className="w-full px-3 py-2 rounded-t-[6px] min-h-[96px] focus:outline-none"
									style={inputStyle}
								/>
							</div>
						</div>
						...
						<div className="flex gap-2 mt-5">
							<button
								...
								disabled={isSaving}
								className="flex-1 py-2.5 text-white rounded-[6px] transition-all hover:brightness-105 disabled:opacity-60"
								style={{
									background:
						...
						<div className="space-y-4">
							<div className="grid grid-cols-2 gap-3">
								...
								<div>
									...
									<select
										value={editProduct.supplierCategoryId}
										onChange={(event) =>
											setEditProduct({
												...editProduct,
									...
									catalogCategories.length > 0 ? (
										<p
											...
											style={{ color: 'rgba(25,28,30,0.55)' }}
										>
											Workspace này chưa có supplier category, nên danh sách
											đang lấy từ master catalog và sẽ tự tạo category tương ứng
											khi lưu product.
										</p>
									) : null}
								</div>
