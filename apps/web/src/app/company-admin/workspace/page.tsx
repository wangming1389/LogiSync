...
import { parseJwtClaims, getStoredAccessToken } from '@/lib/auth';
...
export default function WorkspaceDashboard() {
	...
	useEffect(() => {
		...
		const currentWorkspaceId = typeof claims?.workspaceId === 'string' ? claims.workspaceId : undefined;
		...
		(async () => {
			try {
				...
				if (!mounted) return;
				setLoadError(error instanceof Error ? error.message : 'Không tải được workspace');
			} finally {
	...
	const claims = token ? parseJwtClaims(token) : null;
	const currentWorkspaceId = typeof claims?.workspaceId === 'string' ? claims.workspaceId : undefined;
	const currentWorkspaceSlug = typeof claims?.workspaceSlug === 'string' ? claims.workspaceSlug : undefined;
	...
	return (
		<div className="p-6 space-y-6">
			...
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
				{[
					{ label: 'Workspace ID', value: workspace?.id ?? '—', trend: 'Current tenant', up: true },
					{ label: 'Workspace Type', value: workspace?.type?.toUpperCase() ?? '—', trend: 'Registered type', up: true },
					{ label: 'Enabled Roles', value: String(activeRoles.length), trend: activeRoles.join(', ') || 'None', up: activeRoles.length > 0 },
					{ label: 'Registered At', value: registeredAt ? new Date(registeredAt).toLocaleDateString('vi-VN') : '—', trend: 'From /workspaces', up: true },
				].map((card) => (
					<div
						key={card.label}
			...
			>
				<div className="flex items-center justify-between mb-3">
					<div>
						...
						<p style={{ fontSize: 13, color: 'rgba(25,28,30,0.6)', marginTop: 2 }}>
							Workspace detail is loaded from the Workspace API and matched by the JWT workspaceId.
						</p>
					</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
					{[
						...
						<div key={label} className="rounded-lg p-3" style={{ background: '#F2F4F7' }}>
							...
							<p style={{ fontSize: 14, color: '#191C1E', marginTop: 4 }}>{String(value)}</p>
						</div>
					))}
				</div>
				{loading ? (
					<p className="mt-4 text-sm text-slate-500">Loading workspace data...</p>
				) : null}
			</div>
				...
				<div className="flex flex-wrap gap-2">
					{activeRoles.length > 0 ? (
						activeRoles.map((role) => (
							<span key={role} className="px-3 py-1 rounded-full" style={{ background: '#D3E4F5', color: '#0F4C8A', fontSize: 12, fontWeight: 500 }}>
								{role}
							</span>
						))
					) : (
						<p className="text-sm text-slate-500">No enabled roles found for this workspace.</p>
					)}
				</div>
			</div>
