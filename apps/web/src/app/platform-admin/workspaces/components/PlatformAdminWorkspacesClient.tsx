...
export default function PlatformAdminWorkspacesClient() {
	...
	const fetchWorkspaces = async () => {
		try {
			const query = `?status=${statusFilter}&limit=100`;
	...
	async function enableRole(id: string) {
		...
		try {
			await api.post(`/workspaces/${id}/roles/enable`, {
				roles: rolesToEnable,
			});
			fetchWorkspaces();
			setRoleModal(null);
						...
						>
							<div className="flex justify-between items-start mb-6">
								...
								<div className="text-xs text-slate-500">
									Enabled roles:{' '}
									{(detail.enabledRoles ?? detail.roles ?? []).join(', ') ||
										'None'}
								</div>
							</div>
