...
import { usePlatformHealthQuery } from '@/services/queries/useHealthQueries';
...
export default function PlatformAdminClient() {
	...
	const ready = healthQuery.data?.ready ?? null;
	const live = healthQuery.data?.live ?? null;
	const lastUpdated = healthQuery.data?.fetchedAt ?? null;
	const error =
	...
	const healthyDependencies = services.filter(
		(service) => service.status === 'operational',
	).length;
	const degradedDependencies = services.filter(
		(service) => service.status !== 'operational',
	).length;
		...
		<div className="p-6 space-y-6">
			<div className="flex items-center justify-between">
				...
				<span
					...
					style={{
						background: overallStatus === 'healthy' ? '#C8F0D8' : '#FFEFC6',
						fontSize: 12,
			...
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
				{[
					...
					},
					{
						...
						value: String(healthyDependencies),
						trend: ready?.ready ? 'Ready' : 'Not ready',
						up: ready?.ready ?? false,
					},
					{
						label: 'Degraded dependencies',
						value: String(degradedDependencies),
						trend: live?.alive ? 'Alive' : 'Not alive',
						up: !(live?.alive === false),
					},
					{
						label: 'Last backend timestamp',
						value: formatTime(health?.timestamp),
						trend: 'Polled every 15s',
						up: true,
					},
				].map((card) => (
					<div
