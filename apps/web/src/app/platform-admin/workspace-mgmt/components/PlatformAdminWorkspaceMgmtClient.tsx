...
export default function PlatformAdminWorkspaceMgmtClient() {
					...
					>
						...
						{(revokeModal.activeShipments ?? 0) > 0 && (
							...
							>
								...
								<p style={{ fontSize: 13, color: '#BA1A1A' }}>
									Warning:{' '}
									<strong>{revokeModal.name ?? revokeModal.company}</strong> has{' '}
									<strong>
						...
						<div className="flex gap-2">
							<button
								...
								disabled={
									confirmName !== (revokeModal.name ?? revokeModal.company)
								}
